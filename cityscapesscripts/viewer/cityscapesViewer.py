...
class CityscapesViewer(QtGui.QMainWindow):
    ...
    def initUI(self):
        ...
        # Select image
        selImageAction = QtGui.QAction(QtGui.QIcon(os.path.join(iconDir, 'shuffle.png')), '&Tools',
                                       self)
        ...
        if self.enableDisparity:
            dispAction = QtGui.QAction(QtGui.QIcon(os.path.join(iconDir, 'disp.png')), '&Tools',
                                       self)
        ...
        displayFilepathAction = QtGui.QAction(QtGui.QIcon(os.path.join(iconDir, 'filepath.png')),
                                              '&Tools', self)
    ...
    def loadDisparities(self):
        ...
        if self.dispImg:
            ...
            dispQt = QtGui.QImage(dispNp.data, dispNp.shape[1], dispNp.shape[0],
    ...
    def drawLabels(self, qp, ignore=[]):
        ...
        if self.highlightObj:
            ...
            polyToDraw = self.getPolygon(self.highlightObj) * QtGui.QTransform.fromScale(self.scale,
                                                                                         self.scale)
            qp2.drawPolygon(polyToDraw)
    ...
    def getCityFromUser(self):
        ...
        if 'CITYSCAPES_DATASET' in os.environ:
            ...
            csPath = os.path.join(os.path.dirname(os.path.realpath(__file__)), '..', '../data')
